...
//var appRoot = setAppRoot("modular", "modular");
var appRoot = setAppRoot("", "");
appRoot = appRoot+'demo/';
console.log(appRoot);
...
$(document).ready(function(){
    ...
    $("#pkgListTable").on('click', '.vtr', function(){
       
        vtr_(this);
    });

   $("#ldgListTable").on('click', '.vltr', function(){
       
        vltr_(this);
    });
    //to print receipt
    $("#pkgSlipModal").on('click', '.ptr', function(){
       
        ptr_();
    });
    $("#ldgSlipModal").on('click', '.ptr', function(){
...
function ptr_(){
    ...
    $("#pkgSlipToPrint").css({fontSize:'8px'});
	
    window.print();//trigger the print dialog
	
    $("#pkgSlipToPrint").modal('hide');//dismiss modal
}
function pltr_(){
	//change the font-size
    $("#ldgSlipToPrint").css({fontSize:'8px'});
	
    window.print();//trigger the print dialog
	
    $("#ldgSlipToPrint").modal('hide');//dismiss modal
}
/**
...
function handleLogin(email, password, callback){
    ...
    $.ajax(appRoot+'access/login', {
        ...
        var msg = textStatus + "Log in failed. Please check your internet connection and try again later.";
...
function vtr_(elem){
    ...
    if(ref){
        ...
        //show modal
        $("#pkgSlipModal").modal('show');
        
        //make server request
        $.ajax({
            url: appRoot+"packings/vtr_",
            type: "post",
            data: {ref:ref},
			beforeSend: function() {
			 //show the loading icon
			 $("#pkgReceipt1").html("<div class='row text-center'><i class='fa fa-spinner faa-spin animated'></i> Loading receipt</div>");
			},
            success: function(returnedData){
                if(returnedData.status === 1){
                    ...
                    $("#pkgReceipt").html("Transaction not found");
                }
            },
...
function vltr_(elem){
    ...
    if(ref){
        //show the loading icon
        $("#ldgSlipReceipt").html("<i class='fa fa-spinner faa-spin animated'></i> Loading receipt");
        
        //show modal
        $("#ldgSlipModal").modal('show');
        
        //make server request
        $.ajax({
            url: appRoot+"loadings/vtr_",
            type: "post",
            data: {ref:ref},
            success: function(returnedData){
                if(returnedData.status === 1){
                    $("#ldgReceipt").html(returnedData.loadingSlip);
                }
                
                else{
                    $("#ldgReceipt").html("Transaction not found");
                }
            },
            error: function(XMLHttpRequest, textStatus, errorThrown) { 
                alert("Status: " + textStatus); alert("Error: " + errorThrown); 
