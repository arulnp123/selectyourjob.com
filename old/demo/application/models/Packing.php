...
class Packing extends CI_Model
{
    ...
    public function getAll($orderBy, $orderFormat, $start, $limit)
    {
        if ($this->db->platform() == "sqlite3") {
            ...
            $this->db->select('modular_packings.id, modular_packings.description,  modular_packings.packingOn, modular_packings.projectName, modular_products.cabinetName,  modular_products.partName, 
			SUM(modular_packingLists.quantity) AS quantity,
			modular_loadingLists.loading_id');
		 ...
		 $this->db->group_by('modular_packings.id');
