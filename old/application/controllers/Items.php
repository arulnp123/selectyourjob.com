...
class Items extends CI_Controller{
    ...
    */
    ...
    public function gcoandqty(){
        ...
        if($itemCode){
            $item_info = $this->item->getItemInfo(['file_1'=>$itemCode], ['projectName', 'cabinetName', 'partName', 'quantity']);
     ...
     public function gcoanditem(){
        $json['status'] = 0;
        
        $itemCode = $this->input->get('_iC', TRUE);
        
        if($itemCode){
            $item_info = $this->item->getItemByProject(['projectName'=>$itemCode]);

            if($item_info){
                $json['partName'] = $item_info->projectName;
                $json['cabinetName'] = $item_info->cabinetName;
                $json['availQty'] = (int)$item_info->quantity;
                //$json['prdId'] = (int)$item_info->id;
                $json['status'] = 1;
            }
        }
    ...
    */
    ...
    public function report($prjId)
    {
        //get all packings from db ranging from $from_date to $to_date
   

        $data['allProducts'] = $this->item->getCurrentStock("modular_products.projectName='".$prjId."'");

        $this->load->view('imports/itemReport', $data);
    }
}