...
class Misc extends CI_Controller{
    ...
    public function dldb(){
        ...
        $file_path = BASEPATH . "sqlite/madular.sqlite";//link to db file
        
        $this->output->set_content_type('')->set_output(file_get_contents($file_path));
    }
    
    
    public function bcg($filepath = "", $text="", $size = "20", $orientation = "horizontal", $code_type = "code128", $print = false, $SizeFactor = 1)
    {
    ...
    public function importdb(){
        ...
        if(file_exists(BASEPATH."sqlite/modular.sqlite")){
            copy(BASEPATH."sqlite/modular.sqlite", BASEPATH."sqlite/backups/".time().".sqlite");
        }
        
        $config['upload_path'] = BASEPATH . "sqlite/";//db files are stored in the basepath
        $config['allowed_types'] = 'sqlite';
        $config['file_ext_tolower'] = TRUE;
        $config['file_name'] = "modular.sqlite";
